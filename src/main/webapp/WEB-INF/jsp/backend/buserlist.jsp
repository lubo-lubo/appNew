...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="x_panel">
            <div class="x_title">
                <h2>
                    后台用户列表 <i class="fa fa-user"></i><small>${userSession.userName}
                    - 您可以通过搜索或者其他的筛选项对后台用户信息进行管理。^_^</small>
                </h2>
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                <form method="post" action="/buser/search">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <select name="userTypeId" class="form-control">
                                        <c:if test="${userTypeList != null }">
                                            ...
                                            <c:forEach var="dataDictionary" items="${userTypeList}">
                                                <option <c:if test="${dataDictionary.valueId == userTypeId }">selected="selected"</c:if>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap dataTable no-footer dtr-inline collapsed"
                                ...
                                <tbody>
                                <c:forEach var="buser" items="${buserList }" varStatus="status">
                                    <tr role="row" class="odd">
                                        <td tabindex="0" class="sorting_1">${buser.userCode}</td>
                                        <td>${buser.userName}</td>
                                        <td>${buser.data_dictionary.valueName}</td>
                                        <td>${buser.userPassword}</td>
                                        <td>
                                            <button type="button" class="btn btn-default checkApp"
                                                    appinfoid="${buser.id }" versionid="${appInfo.versionId }" status="${appInfo.status }"
...
<script src="${pageContext.request.contextPath }/statics/localjs/buserlist.js"></script>
